...
class omsViewSettings(Document):
	pass
@frappe.whitelist()
def create(view):
	...
	doc = frappe.new_doc("oms View Settings")
	doc.name = view.label
...
def update(view):
	...
	doc = frappe.get_doc("oms View Settings", view.name)
	doc.label = view.label
	doc.type = view.type or 'list'
	doc.icon = view.icon
	doc.route_name = view.route_name or ""
...
@frappe.whitelist()
def delete(name):
	if frappe.db.exists("oms View Settings", name):
		frappe.delete_doc("oms View Settings", name)
...
def public(name, value):
	...
	doc = frappe.get_doc("oms View Settings", name)
	if doc.pinned:
		doc.pinned = False
	doc.public = value
...
def create_or_update_default_view(view):
	...
	if doc:
		doc = frappe.get_doc("oms View Settings", doc)
		doc.label = view.label
		doc.type = view.type or 'list'
		doc.route_name = view.route_name or ""
		doc.load_default_columns = view.load_default_columns or False
