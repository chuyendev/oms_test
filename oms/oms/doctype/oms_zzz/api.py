...
import json
from oms.api.doc import get_fields_meta


@frappe.whitelist()
def create_zzz(ho_va_ten,kieu_check,kieu_date,kieu_datetime,kieu_float,kieu_int,kieu_rating,kieu_select,kieu_small_text,kieu_text,kieu_link,kieu_text_editor,kieu_attach):
    try:
        zzz_record = frappe.get_doc({
            "doctype": "oms_zzz",
            "ho_va_ten": ho_va_ten,
        ...
        return {
            ...
            "data": zzz_record.name  # Trả về tên (ID) của bản ghi mới
        }
...
@frappe.whitelist()
def get_zzz(name):
	lst = frappe.qb.DocType("oms_zzz")

	query = frappe.qb.from_(lst).select("*").where(lst.name == name).limit(1)

	lst_data = query.run(as_dict=True)
	if not len(lst_data):
		frappe.throw(_("zzz not found"), frappe.DoesNotExistError)
	lst_data = lst_data.pop()

	lst_data["doctype"] = "oms_zzz"
	lst_data["fields_meta"] = get_fields_meta("oms_zzz")
	print("Test log lst_data : ",lst_data)
	return lst_data


@frappe.whitelist()
def update_zzz(zzzId,ho_va_ten=None,kieu_check=None,kieu_date=None,kieu_datetime=None,kieu_float=None,kieu_int=None
               ,kieu_rating=None,kieu_select=None,kieu_small_text=None,kieu_text=None,kieu_link=None,kieu_text_editor=None,kieu_attach=None):
    # Lấy bản ghi cần cập nhật
    doc = frappe.get_doc('oms_zzz', zzzId)
    ...
    if kieu_attach:
        doc.kieu_attach = kieu_attach 
    doc.save()
    frappe.db.commit()
    return {"message": "zzz updated successfully"}