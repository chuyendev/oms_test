...
class omsFieldsLayout(Document):
	pass
@frappe.whitelist()
def get_fields_layout(doctype: str, type: str):
	sections = []
	if frappe.db.exists("oms Fields Layout", {"dt": doctype, "type": type}):
		layout = frappe.get_doc("oms Fields Layout", {"dt": doctype, "type": type})
	else:
		return []
...
def save_fields_layout(doctype: str, type: str, layout: str):
	if frappe.db.exists("oms Fields Layout", {"dt": doctype, "type": type}):
		doc = frappe.get_doc("oms Fields Layout", {"dt": doctype, "type": type})
	else:
		doc = frappe.new_doc("oms Fields Layout")
